...
def run_track_and_save(
    ...
    try:
        for result in model.track(video_path, stream=True, conf=conf, imgsz=imgsz, tracker=tracker_yaml, verbose=False):
            ...
            # [핵심 수정 1] 깨끗한 원본(clean_frame)과 시각화용(vis_frame) 분리
            clean_frame = result.orig_img  # YOLO 결과의 원본 이미지 (수정 금지)
            ...
            # [핵심 수정 2] ROI 그리기 -> vis_frame에만 그리기
            ...
            for p in persons:
                ...
                for c in cars:
                    ...
                    score = 0.1*s_iou + 0.8*s_dist + 0.1*s_time

                    if score > best_score:
                        best_score, best_car = score, cid

                if best_car and best_score > 0.9:
            ...
            if save_crops:
                for d in dets:
                    ...
                    # [핵심 수정 3] 크롭은 반드시 clean_frame에서 수행
